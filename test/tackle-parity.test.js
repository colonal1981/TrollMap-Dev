...
describe('depth model — one source, four modes', () => {
  ...
  // THE EXCEPTION LIST IS EMPTY, AND IT WAS EMPTIED THE RIGHT WAY.
  //
  // For a few hours on 2026-09-14 this held `cast_fluke`: Ryan had ruled it trollable ("rig it up
  // with either a belly weight or a jighead and now it does troll") while its type was `cast_only`,
  // depthMode 'none', shared with three baits he ruled the other way. The fix was not to widen the
  // invariant -- it was to file the fluke where its behaviour lives, `swimbait_paddle`, whose whole
  // meaning is "soft plastic, the head IS the weight". The list stays here, empty, because an
  // invariant with a documented escape hatch is how the next one gets waved through.
  const TROLLS_BUT_CANNOT_BE_PLACED = new Set([]);
  ...
  it('and the exception list is EMPTY, so nothing is riding on a waiver', () => {
    expect([...TROLLS_BUT_CANNOT_BE_PLACED]).toEqual([]);
  });
