...
registry/_nc_species_unmatched.json. If any is missing it is reported as SKIPPED, naming the
file, through registry_here.py -- never passed, because a test that cannot find its data and
passes forever is the failure this project already had. test_registry_here.py fails if the
registry is on disk and that helper cannot see it.
...
"""
import collections, importlib.util, io, json, os, sys, unittest
...
NEED = ('lake_index.json', '_nc_species_cache.json', '_nc_species_unmatched.json', 'nc_species_by_lake.json',)
...
class TheLastRun(unittest.TestCase):

    def test_the_bank_pad_and_the_ncpaws_launches(self):
        _run()
