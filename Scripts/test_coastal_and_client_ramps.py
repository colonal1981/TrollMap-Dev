...
No network. Reads registry/lake_index.json and the four saved _dnr_*.json feeds. Without the index
it is reported as SKIPPED, with that reason (registry_here.py), rather than passing forever.
...
class TheRealIndex(unittest.TestCase):
    ...
    def test_coastal_zones_and_client_ramps(self):
        _run()
