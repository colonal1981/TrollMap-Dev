...
function renderDebugPanel() {
  ...
  if (!panel) {
    ...
    document.getElementById('_routeDebugCopy').onclick = () => {
      const data = { timestamp: new Date().toISOString(), phases: window._routeDebug || [], spineScores: window._routeDebugScores || [] };
      navigator.clipboard.writeText(JSON.stringify(data, null, 2))
        .then(() => { document.getElementById('_routeDebugCopy').textContent = '✅ Copied!'; setTimeout(() => { document.getElementById('_routeDebugCopy').textContent = '📋 Copy JSON'; }, 2000); })
        .catch(() => { document.getElementById('_routeDebugCopy').textContent = '❌ Failed'; setTimeout(() => { document.getElementById('_routeDebugCopy').textContent = '📋 Copy JSON'; }, 2000); });
    };
...
// NO TOOLBAR BUTTON. Ryan, 2026-08-07: "spine debug needs to be removed from the bottom
// button panel." This module used to append its own button to `.map-toolbar` on a
// DOMContentLoaded + 2 s timer, which is why it appeared beside the others without being
// in index.html and why it could not be removed by editing the markup.
//
// The panel itself is kept and still reachable, because route-builder.js:1191 feeds it and
// it is the only view into what the spine actually did. To open it:
//
//     document.getElementById('_routeDebugPanel').style.display = 'block'
//
// If the whole thing should go, it is this file, the main.js import, and the
// `typeof window.logSpineDebug === 'function'` guard in route-builder.js. See
// DELETION_TAB.md -- it is listed there with that scope.
