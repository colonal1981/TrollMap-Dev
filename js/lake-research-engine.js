...
async function runFisheriesRefresh(lakeName, callbacks = {}) {
  ...
  try {
    ...
    await new Promise(res => setTimeout(res, 2000));
    // Run trolling agent with enriched biology — strip large fields to keep payload small
    const trollingInput = {
      identity:   agentInput.identity   || existingProfile.identity,
      limnology:  agentInput.limnology  || existingProfile.limnology,
      habitat:    agentInput.habitat    || existingProfile.habitat,
      biology:    newBiology,
      // exclude _extractedFacts and _documentContext — trolling agent only needs the profile
    };
